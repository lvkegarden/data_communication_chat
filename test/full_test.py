...
class BrowserDoubaoClient:
    ...
    def query_competitors(self, product_name: str, product_code: str, brands: List[str] = ['华为', 'H3C', '锐捷'] ) -> Dict[str, Any]:
        ...
        query = f"{product_code} 产品 竞品分析，给出 {brands_str} 三个品牌的竞品名称"
...
锐捷:RG-S5750-24GT4XS-P-L"""
    ...
    def _guess_brand(self, product_code: str) -> Optional[str]:
        ...
        猜测产品品牌
        """
        if product_code.startswith('S') and not product_code.startswith('S6'):
            return '华为'
        elif product_code.startswith('S5') or product_code.startswith('S6') or product_code.startswith('WX'):
            return 'H3C'
    ...
    def _get_mock_competitors(self, product_code: str, brands: List[str]) -> Dict[str, Any]:
        ...
        """
        mock_data = {
            'S5735-L-V2': {'H3C': ['S5130S-28P-EI', 'S5130S-52P-EI'], '锐捷': ['RG-S5750-24GT4XS-P-L', 'RG-S5760-24GT4XS-P']},
            'S5130S-EI': {'华为': ['S5735-L-V2', 'S5735-S-V2'], '锐捷': ['RG-S5310-E', 'RG-S5750-E']},
            'RG-S5750-L': {'华为': ['S5735-L-V2', 'S5735-S-V2'], 'H3C': ['S5130S-LI', 'S5130S-SI']}
        }
        ...
        if product_code in mock_data:
            ...
            if target_brand and target_brand in default_mapping:
                for brand in brands:
                    if brand != target_brand and brand in default_mapping[target_brand]:
                        competitors.append({
                            'brand': brand,
                            'code': default_mapping[target_brand][brand]
                        })
...
class FullCompetitorComparisonTest:
    """
    全量竞品分析对比测试
    """
    ...
    def run_tests(self, test_products: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if summary['total_products'] > 0:
            ...
            for brand in summary['by_brand']:
                count = summary['by_brand'][brand]['count']
                if count > 0:
                    summary['by_brand'][brand]['precision'] = round(summary['by_brand'][brand]['precision'] / count, 4)
                    summary['by_brand'][brand]['recall'] = round(summary['by_brand'][brand]['recall'] / count, 4)
                    summary['by_brand'][brand]['f1'] = round(summary['by_brand'][brand]['f1'] / count, 4)
    ...
    def save_results(self, results: Dict[str, Any], filename: str = 'full_doubao_comparison_results.json'):
        """
        保存结果到文件
        """
...
def main():
    ...
    主函数
    """
    test = FullCompetitorComparisonTest()
    
    print(f"\n{'=' * 80}")
    print(f"全量豆包竞品分析对比测试")
    print(f"{'=' * 80}")
    
    # 抽取所有测试产品
    test_products = test.extract_all_test_products()
    ...
    if test_products:
        ...
        print(f"\n按品牌统计:")")
        for brand, brand_stats in summary['by_brand'].items():
            print(f"\n  {brand}:")")
            print(f"    测试数: {brand_stats['count']}")")
            print(f"    匹配数: {brand_stats['matched']}")")
            print(f"    遗漏数: {brand_stats['missed']}")")
            print(f"    额外数: {brand_stats['extra']}")")
            print(f"    Precision: {brand_stats['precision']:.4f}")")
            print(f"    Recall: {brand_stats['recall']:.4f}")")
            print(f"    F1: {brand_stats['f1']:.4f}")")
