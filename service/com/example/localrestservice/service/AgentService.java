...
public class AgentService {
    ...
    public Map<String, Object> chat(String sessionId, String userMessage, String flag) {
        return chat(sessionId, userMessage, flag, "qwen");
    }

    public Map<String, Object> chat(String sessionId, String userMessage, String flag, String modelProvider) {
        ...
        try {
            ...
            String requestBody = String.format("{\"session_id\":\"%s\",\"message\":\"%s\",\"flag\":\"%s\",\"model_provider\":\"%s\"}", 
                    escapeJson(sessionId), escapeJson(userMessage), escapeJson(flag), escapeJson(modelProvider));
            ...
            if (success) {
                ...
                String responseModelProvider = responseJson.path("model_provider").asText();
                String responseModelName = responseJson.path("model_name").asText();

                result.put("success", true);
                result.put("message", assistantMessage);
                result.put("sessionId", responseSessionId);
                result.put("modelProvider", responseModelProvider);
                result.put("modelName", responseModelName);
